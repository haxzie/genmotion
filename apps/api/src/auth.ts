  ...
  plugins: [
    ...
    organization({
      ...
      organizationHooks: {
         ...
         * invitation row is written and before the email is sent — so
         * throwing here rejects the invite cleanly, leaving no row and
         * sending nothing. The gate is the team policy in entitlements.ts:
         * Pro has no teammates (the answer is Max), Max has five seats (the
         * answer past that is a conversation). Seats are not bought here.
         */
        beforeCreateInvitation: async ({ invitation }) => {
          const gate = await assertCanInvite(invitation.organizationId);
          if (gate.ok) return;
          throw APIError.from("FORBIDDEN", { code: gate.code, message: gate.message });
        },

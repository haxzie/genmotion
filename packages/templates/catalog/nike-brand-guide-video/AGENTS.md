...
The swoosh mark draws on, the palette and type specimen unfold, then application shots orbit into a tagline lockup, wrapping a full brand guide into one sequence.

Started from the **Nike brand guide video** template. A GenMotion video project. Scenes are React components rendered frame by frame

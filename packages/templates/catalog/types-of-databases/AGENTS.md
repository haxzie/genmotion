...
A hand drawn whiteboard explainer with voiceover: six ways to store data, from relational tables and documents to key value, graph, time series and vector, each one sketched as it is narrated.

Started from the **Types of databases explainer video** template. A GenMotion video project powered by the Three.js engine. Scenes are plain
